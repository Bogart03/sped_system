                ...
                <table class="table table-bordered table-striped table-hover text-center align-middle">
                    ...
                    <tbody>
                        ...
                        $qry = $conn->query("SELECT q.*, u.name as fname FROM quiz_list q 
                                             ...
                                             $where ORDER BY q.title ASC");
                        while($row = $qry->fetch_assoc()):
                            $total_questions = $conn->query("SELECT COUNT(id) as cnt FROM questions WHERE qid = '".$row['id']."'")->fetch_array()['cnt'];

                            $taken_count = 0;
                            if($_SESSION['login_user_type'] == 3){
                                $taken_count = $conn->query("SELECT COUNT(id) as cnt FROM answers 
                                                             WHERE quiz_id = '".$row['id']."' 
                                                             AND user_id = '".$_SESSION['login_id']."'")->fetch_array()['cnt'];
                                $status = ($taken_count == $total_questions && $total_questions > 0) ? 'taken' : 'pending';
                            } else {
                                $taken_count = $conn->query("SELECT COUNT(DISTINCT user_id) as cnt 
                                                             FROM answers WHERE quiz_id = '".$row['id']."'")->fetch_array()['cnt'];
                            }
                        ?>
                        <tr>
                            ...
                            <td>
                                🔢 <i class="fa-solid fa-circle-dot item-icon me-1"></i>
                            ...
                            <?php if($_SESSION['login_user_type'] == 3): ?>
                                <td>
                                    <?php 
                                    $progress = ($total_questions > 0) ? round(($taken_count / $total_questions) * 100) : 0;
                                    ?>
                                    <?php if($status == 'taken'): ?>
                                    ...
                                    <!-- Progress bar -->
                                    <div class="progress mt-2" style="height: 18px; border-radius: .5rem;">
                                        <div class="progress-bar <?php echo ($progress == 100) ? 'bg-success' : 'bg-info'; ?>" 
                                             ...
                                             aria-valuemin="0" 
                                             aria-valuemax="100">
                                            <?php echo $taken_count; ?>/<?php echo $total_questions; ?> (<?php echo $progress; ?>%)
                                        </div>
                                    </div>
                                </td>
                            <?php else: ?>
                                <td>
                                    <span class="badge bg-primary">
                                        <i class="fa-solid fa-users me-1"></i> <?php echo $taken_count; ?> Students
                                    </span>
                                </td>
                            <?php endif; ?>
                        </tr>
